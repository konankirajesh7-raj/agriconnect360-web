...
export default function NetworkPage() {
  ...
  const filteredPeople = ALL_PEOPLE.filter(p => {
    ...
    if (search && !p.name.toLowerCase().includes(search.toLowerCase()) && !p.loc.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="animated">
      <div className="section-header">
        <div>
          ...
          <div style={{ fontSize:'0.75rem', color:'var(--text-muted)', marginTop:2 }}>Connect with farmers, labourers, brokers, suppliers & industries</div>
        </div>
      ...
      {tab === 'connections' && (
        <div>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Search by name or district..." style={{ ...INP, marginBottom:10 }} />
