...
export default function AdvertisePage() {
  ...
  const submitAd = async () => {
    if (!form.title || !form.media) { setToast('�a�️ Please add title and media'); setTimeout(() => setToast(''), 2500); return; }
    setPayStep(1);
  };
  ...
  return (
    <div className="animated" style={{ padding: '20px 24px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {[{ id: 'create', icon: '�~"', label: 'Create Ad' }, { id: 'my', icon: '�x9', label: 'My Ads' }, { id: 'browse', icon: '�x�', label: 'Active Ads' }].map(t => (
      ...
      {tab === 'create' && payStep === 0 && (
        <div style={{ ...G.glass, borderRadius: 16, padding: 24 }}>
          <div style={{ fontWeight: 700, color: '#f1f5f9', marginBottom: 18, fontSize: '1rem' }}>�x� Create Advertisement</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button onClick={submitAd} style={{ padding: '12px 32px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: '#000', fontWeight: 700, cursor: 'pointer', fontSize: '0.92rem' }}>�x� Proceed to Pay ��{form.duration === '7' ? '50' : form.duration === '15' ? '90' : '150'}</button>
      ...
      {tab === 'create' && payStep === 1 && (
        <div style={{ ...G.glass, borderRadius: 16, padding: 32, textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, maxWidth: 400, margin: '0 auto 24px' }}>
            {[{ icon: '�x�', label: 'UPI / GPay' }, { icon: '�x��', label: 'Net Banking' }, { icon: '�x�', label: 'Card' }].map(m => (
          ...
          <button onClick={processPayment} disabled={processing} style={{ padding: '14px 40px', borderRadius: 14, border: 'none', background: processing ? '#475569' : 'linear-gradient(135deg, #22c55e, #16a34a)', color: '#fff', fontWeight: 700, cursor: processing ? 'default' : 'pointer', fontSize: '0.95rem' }}>
            {processing ? '⏳ Processing Payment...' : '�S& Pay & Submit Ad'}
          </button>
      ...
      {tab === 'create' && payStep === 2 && (
        <div style={{ ...G.glass, borderRadius: 16, padding: 40, textAlign: 'center' }}>
          <div style={{ fontSize: '4rem', marginBottom: 12 }}>�x}0</div>
          ...
          <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.75rem', marginTop: 8 }}>Payment of ��{form.duration === '7' ? '50' : form.duration === '15' ? '90' : '150'} received �S</div>
        </div>
      )}
      ...
      {tab === 'my' && (
        <div style={{ ...G.glass, borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', fontWeight: 700, color: '#f1f5f9' }}>�x9 My Advertisements ({myAds.length})</div>
