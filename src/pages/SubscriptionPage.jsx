...
const ROLE_ICONS = {
  customer:'�x:', farmer:'�x�⬍�xR�', supplier:'�x��', broker:'�x��', labour:'�x�', industrial:'�x��', admin:'�a"️',
};
...
export default function SubscriptionPage() {
  ...
  const savePlans = async () => {
    ...
    try {
      ...
      setPlans(editPlans);
      setSavedMsg('�S& Plans saved successfully!');
      setEditMode(false);
      setTimeout(() => setSavedMsg(''), 3000);
    } catch (err) {
      setSavedMsg('�R Error saving: ' + err.message);
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="animated">
      ...
      {isAdmin && (
        <>
          <div style={{ display:'flex', gap:6, marginBottom:20 }}>
            {[['plans','�x9 Pricing Plans'],['subscribers','�x� Subscribers'],['revenue','�x� Revenue']].map(([id, label]) => (
          ...
          <div className="grid-4" style={{ marginBottom:20 }}>
            {[
              { l:'Total Revenue', v:`��${revenue.total.toLocaleString()}`, i:'�x�', c:'#22c55e' },
              { l:'Pending Dues', v:`��${revenue.pending.toLocaleString()}`, i:'⏳', c:'#f59e0b' },
              { l:'Active Subscribers', v:revenue.active, i:'�S&', c:'#3b82f6' },
        ...
        <div>
          {isAdmin && (
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div style={{ fontWeight:700 }}>�x9 Role Pricing Configuration</div>
              <div style={{ display:'flex', gap:8 }}>
                {savedMsg && <span style={{ fontSize:'0.82rem', color: savedMsg.startsWith('�S&') ? '#22c55e' : '#f87171', fontWeight:600 }}>{savedMsg}</span>}
                {editMode ? (
                  <>
                    <button onClick={savePlans} disabled={saving} style={{ padding:'8px 18px', borderRadius:8, background:'linear-gradient(135deg,#22c55e,#16a34a)', color:'#fff', border:'none', fontWeight:700, cursor:'pointer', fontSize:'0.82rem' }}>{saving ? '⏳ Saving...' : '�S& Save Changes'}</button>
                  ...
                  <button onClick={() => setEditMode(true)} style={{ padding:'8px 18px', borderRadius:8, background:'linear-gradient(135deg,#6366f1,#8b5cf6)', color:'#fff', border:'none', fontWeight:700, cursor:'pointer', fontSize:'0.82rem' }}>�S�️ Edit Prices</button>
                )}
              </div>
            </div>
          )}
            ...
            {Object.entries(isAdmin && editMode ? editPlans : plans).map(([role, plan]) => (
              <div key={role} className="card" style={{ padding:20, border: role === userRole ? '1px solid var(--accent)' : '1px solid var(--border)' }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {plan.free_forever ? (
                    <span style={{ fontSize:'0.72rem', color:'#22c55e' }}>�S All features included</span>
                  ) : (
                    <>
                      ...
                      <span style={{ padding:'2px 8px', borderRadius:6, background:'rgba(34,197,94,0.1)', color:'#22c55e', fontSize:'0.68rem', fontWeight:600 }}>�x� UPI / Card</span>
                    </>
                  )}
                </div>
      ...
      {isAdmin && tab === 'revenue' && (
        <div className="card" style={{ padding:32, textAlign:'center' }}>
          <div style={{ fontSize:'3rem', marginBottom:12 }}>�x�</div>
